...
const fs = require('fs')
const consolePrefix = `\x1b[34m[\x1b[33mnovacord-ui\x1b[34m]\x1b[36m `
...
async function update() {
    ...
    try {
        await fetch(`https://cdn.jsdelivr.net/gh/Assistants-Center/Novacord-UI/utils/updater/versionsOnline.json`);
    } catch (error) {
        failed3++
    ...
    if (failed3 === 0) {
        let checkArray = await fetch(`https://cdn.jsdelivr.net/gh/Assistants-Center/Novacord-UI/utils/updater/versionsOnline.json`);
        ...
        if (failed4 === 0) {
            ...
            for (const latestFile of checkArray) {
                if (latestFile.version > currentVersions[latestFile.name]) {
                    ...
                    if (type === 'partial') {
                        let failedFile = 0
                        let fileRaw = await fetch(`https://cdn.jsdelivr.net/gh/Assistants-Center/Novacord-UI/views/partials/${name}.ejs`);
